...
def main() -> int:
    ...
    if args.geometry_json and args.geometry_json.is_file():
        ...
        geometry = {str(k): float(v) for k, v in (raw.get("geometry") or raw).items()}
        shape_name = str(raw.get("shape_name") or shape_name)
    elif args.run_dir:
        sample = _load_sample_input(args.run_dir.expanduser().resolve())
        geometry = extract_geometry_dict(sample)
        shape_name = str(sample.get("shape_name") or shape_name)
    else:
