...
def export_region_dof_indices_npz(
    ...
    *,
    ...
    """Best-effort region DOF index export. Returns (status, error_message)."""
    ...
    def _trace_u_rows(facets: np.ndarray) -> np.ndarray:
        ...
        # B3 u_idx is identity 0..n_u-1 on trace structural DOFs
...
def write_stage_a_synthesis_artifacts(
    ...
    *,
    ...
    try:
        region_status, region_error = export_region_dof_indices_npz(
            checkpoint,
        ...
        region_status = "deferred_to_stage_c"
        region_error = f"{type(exc).__name__}:{exc}"
