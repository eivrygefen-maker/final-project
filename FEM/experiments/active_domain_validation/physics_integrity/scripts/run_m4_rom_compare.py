    ...
    for row in selection:
        ...
        if comparison and comparison.get("status") == "COMPLETED":
            ...
            print(
                ...
                f"rad_log_mae={p2.get('radiation_proxy_log_mae')} "
                f"mic_norm_mae={p2.get('mic_output_proxy_p95_norm_mae')} "
                f"rad_rank={p2.get('radiation_proxy_rank_correlation')} "
                f"topk_rad={p2.get('top_k_radiation_overlap')} "
                f"rad_rel_diag={p2.get('radiation_proxy_relative_error_median')} "
