...
TMP_ROOT="$(mktemp -d "${TMPDIR:-/tmp}/classic_fom_gmsh_demo_${SAMPLE_ID}.XXXXXX")"
...
DEMO_MESH="${TMP_ROOT}/FEM/mesh/sample_000_full_fom_guitar_3d.msh"
...
mesh_path = tmp / "FEM" / "mesh" / "sample_000_full_fom_guitar_3d.msh"
...
config.setdefault("solver", {})["mesh_file"] = str(mesh_path)
...
print(f"Temporary FOM config: {out_path}")
print(f"Temporary FOM mesh: {mesh_path}")
PY

cat <<'TXT'

Full-FOM Classical GMSH demo
----------------------------
This uses the repository geometry builder's FEM_ALLOW_FOM=1 path.
It generates a full FSI/FOM mesh only; it does not run a FEM solve, ROM, STK, WAV, or website pipeline.

Physical group legend used by the real FOM model where present:
  Surface 1: Top_Plate
  Surface 2: Soundhole
  Surface 3: Back_Plate
  Surface 4: Ribs_Sides
  Surface 5: wood_fix / support
  Volume 1:  Top_Plate_Volume
  Volume 2:  Back_Plate_Volume
  Volume 3:  Ribs_Sides_Volume
  Volume 10: Air_Internal

In GMSH:
  - Use the Visibility/Physical Groups panels to toggle groups.
  - Hide outer wood surfaces or use clipping/section view to reveal the internal air volume.
  - Toggle Mesh visibility to inspect tetrahedra and boundary triangles.
...
echo "Generating temporary full FOM GMSH mesh. This may take noticeably longer than the display demo..."

(
  cd "${TMP_ROOT}"
  FEM_ALLOW_FOM=1 FEM_MESH_OUT="${DEMO_MESH}" "${PYTHON_BIN}" "FEM/geometry/build_3d_guitar.py" --config "${DEMO_CONFIG}" -nopopup
)

if [[ ! -s "${DEMO_MESH}" ]]; then
  echo "ERROR: expected temporary FOM mesh was not created: ${DEMO_MESH}" >&2
  exit 1
fi

echo "Opening GMSH GUI for full-FOM ${SAMPLE_ID}: ${DEMO_MESH}"
