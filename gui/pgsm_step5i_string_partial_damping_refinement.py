#!/usr/bin/env python3
"""
PGSM Step 5I — string partial damping refinement.
Refined string-force partial decay driving unchanged Step 3C modal body; diagnostic only.
"""
from __future__ import annotations

import hashlib
import json
import math
from datetime import datetime, timezone
from pathlib import Path
from typing import Any, Dict, List, Mapping, Optional, Sequence, Tuple

import numpy as np

from pgsm_physical_factor_registry import helmholtz_proxy_hz
from pgsm_step2_1_parameter_targets import load_step_report
from pgsm_step3a_numerical_ir_testbench import (
    DURATION_S,
    FIXED_PLUCK_POSITION,
    NUMERIC_SR,
    SAMPLE_ID,
    compute_impulse_response,
)
from pgsm_step4a_single_note_diagnostic_audio import (
    DIAGNOSTIC_LABEL,
    build_calibrated_modal_state,
    normalize_diagnostic_amplitude,
    synthesize_modal_body_response,
    write_wav_mono,
)
from pgsm_step4b_single_note_diagnostic_refinement import _envelope, load_wav_mono
from pgsm_step5a_limited_note_set_diagnostic_audio import NOTE_FREQUENCY_HZ, NOTE_SET
from pgsm_step5e_string_driven_bridge_force_repair import (
    ACTIVE_DURATION_MIN_MS_HIGH,
    ACTIVE_DURATION_MIN_MS_LOW,
    ENERGY_FIRST_10MS_MAX,
    INHARMONICITY_B_FALLBACK,
    INHARMONICITY_LEVEL,
    OUTPUT_DURATION_S,
    PITCH_SALIENCE_MIN,
    TARGET_RMS_DBFS_MAX,
    TARGET_RMS_DBFS_MIN,
    TARGET_RMS_DBFS_NOMINAL,
    apply_listening_render_full,
    build_artifact_guard,
    build_string_driven_bridge_force,
    collect_previous_audio_fingerprints,
    compute_click_dominance_score,
    compute_harmonic_energies,
    compute_modal_kernels_decomposed,
    compute_pitch_salience,
    compute_spectral_features,
    detect_second_onset_sustained,
    evaluate_modal_peak_alignment,
    _active_duration_ms,
    _decay_time_ms_smoothed,
    _energy_share_first_ms,
    _linear_to_dbfs,
    _rms,
)
from pgsm_step5f_string_driven_extended_validation import (
    READINESS_AFTER as READINESS_STEP5F,
    collect_step5e_fingerprints,
    compute_hnr_proxy,
    compute_partial_decay_slopes,
    compute_spectral_centroid_over_time,
    step5e_wav_paths,
)
from pgsm_step5h_note_string_fret_contract import READINESS_AFTER as READINESS_STEP5H
from stk_pipeline_defaults import DEFAULT_WEBSITE_STK_MODE

PGSM_STEP5I_VERSION = "pgsm_step5i_string_partial_damping_refinement_v1"
REPO_ROOT = Path(__file__).resolve().parents[1]
REPORT_JSON = REPO_ROOT / "audio" / "debug_reports" / "pgsm_step5i_string_partial_damping_refinement.json"
REPORT_MD = REPO_ROOT / "audio" / "debug_reports" / "pgsm_step5i_string_partial_damping_refinement.md"
DATA_JSON = REPO_ROOT / "data" / "pgsm_string_partial_damping_contract.json"
AUDIO_DIR = REPO_ROOT / "audio" / "pgsm_step5i_string_partial_damping_refinement"

READINESS_AFTER = "ready_for_step5j_top_back_air_radiation_weighting_refinement"
STRING_FORCE_LABEL = "diagnostic_refined_string_bridge_force_proxy_not_measured_force"

BASE_TAU_BY_STRING: Dict[str, float] = {
    "string_6": 3.40,
    "string_5": 3.15,
    "string_4": 2.85,
    "string_3": 2.65,
    "string_2": 2.35,
    "string_1": 2.05,
}

MATERIAL_INTERNAL_LOSS: Dict[str, float] = {
    "string_6": 1.00,
    "string_5": 1.02,
    "string_4": 1.04,
    "string_3": 1.06,
    "string_2": 1.08,
    "string_1": 1.10,
}

HARMONIC_ORDER_EXPONENT = 0.92
FREQ_DEPENDENT_COEFF = 1.6e-4
FRET_CONTACT_COEFF = 0.028
BOUNDARY_LOSS_COEFF = 0.032
STEP5E_HARMONIC_EXPONENT = 0.65
STEP5E_BASE_TAU = 2.8


def _utc_now() -> str:
    return datetime.now(timezone.utc).strftime("%Y-%m-%dT%H:%M:%SZ")


def _report_path(root: Path, name: str) -> Path:
    return root / "audio" / "debug_reports" / name


def _file_fingerprint(path: Path) -> str:
    if not path.is_file():
        return ""
    return hashlib.sha256(path.read_bytes()).hexdigest()


def collect_all_previous_audio_fingerprints(root: Path) -> Dict[str, str]:
    return {
        **collect_previous_audio_fingerprints(root),
        **collect_step5e_fingerprints(root),
    }


def _modal_state_fingerprint(state: Mapping[str, Any]) -> str:
    modes = state.get("modal_weights", {}).get("modes") or []
    parts = [
        f"{m.get('frequency_hz')}:{m.get('tau_s')}:{m.get('Q_total')}"
        for m in modes[:24]
    ]
    return hashlib.sha256("|".join(parts).encode()).hexdigest()


def build_string_partial_damping_contract() -> Dict[str, Any]:
    terms: List[Dict[str, Any]] = [
        {
            "term": "base_string_decay_by_string_id",
            "formula": "tau_base[string_id] seconds (literature-proxy open-string sustain)",
            "values": dict(BASE_TAU_BY_STRING),
            "source_level": "L2_literature_fallback",
            "units": "seconds",
            "limitations": "Not measured per-string damping; bass strings assigned longer base tau.",
            "blocked_claims": ["measured_string_damping", "exact_nylon_loss_coefficient"],
            "objective_metric": "tau_base differs across string_id",
        },
        {
            "term": "harmonic_order_loss",
            "formula": f"harmonic_factor = k^{HARMONIC_ORDER_EXPONENT}",
            "source_level": "L2_diagnostic_proxy",
            "units": "dimensionless divisor on tau",
            "limitations": "Steeper than Step 5E k^0.65; not validated against measurements.",
            "blocked_claims": ["validated_partial_Q_per_harmonic"],
            "objective_metric": "tau_k decreases monotonically with harmonic order k",
        },
        {
            "term": "frequency_dependent_loss",
            "formula": f"freq_factor = 1 + {FREQ_DEPENDENT_COEFF} * f_k",
            "source_level": "L2_diagnostic_proxy",
            "units": "dimensionless divisor on tau",
            "limitations": "High-frequency partials lose energy faster; not arbitrary EQ.",
            "blocked_claims": ["spectral_eq_shaping"],
            "objective_metric": "higher f_k yields shorter tau_k",
        },
        {
            "term": "fret_contact_loss_proxy",
            "formula": f"fret_factor = 1 + {FRET_CONTACT_COEFF} * fret",
            "source_level": "L2_diagnostic_proxy",
            "units": "dimensionless divisor on tau",
            "limitations": "Fretted notes include modest contact damping vs open strings.",
            "blocked_claims": ["measured_fret_contact_impedance"],
            "objective_metric": "fret>0 increases damping vs open string on same string",
        },
        {
            "term": "material_internal_loss_proxy",
            "formula": "material_factor = MATERIAL_INTERNAL_LOSS[string_id]",
            "source_level": "L2_nylon_classical_literature_fallback",
            "units": "dimensionless divisor on tau",
            "limitations": "Treble strings proxy shorter sustain; not measured linear density loss.",
            "blocked_claims": ["measured_internal_friction"],
            "objective_metric": "treble string_id has larger material_factor",
        },
        {
            "term": "bridge_nut_boundary_loss_proxy",
            "formula": f"boundary_factor = 1 + {BOUNDARY_LOSS_COEFF} * sqrt(k)",
            "source_level": "L2_diagnostic_proxy",
            "units": "dimensionless divisor on tau",
            "limitations": "Boundary reflection loss increases with partial order proxy.",
            "blocked_claims": ["measured_bridge_admittance_per_partial"],
            "objective_metric": "boundary_factor increases with k",
        },
        {
            "term": "combined_partial_tau",
            "formula": (
                "tau_k = tau_base / (harmonic_factor * freq_factor * fret_factor "
                "* material_factor * boundary_factor)"
            ),
            "source_level": "L2_diagnostic_combined_proxy",
            "units": "seconds",
            "limitations": "Diagnostic physics proxy only; replaces Step 5E global tau_k in Step 5I generator.",
            "blocked_claims": ["real_guitar_equivalence", "validated_damping_model"],
            "objective_metric": "per-note tau table; high k shorter tau than low k",
        },
    ]
    return {
        "contract_id": "pgsm_string_partial_damping_v1",
        "implements_multi_decay_term": "string_partial_decay_only",
        "not_implemented_terms": [
            "top_plate_decay",
            "back_plate_decay",
            "air_cavity_decay",
            "radiation_decay",
            "bridge_coupling_loss",
        ],
        "step5e_replaced_law": f"tau_k = {STEP5E_BASE_TAU} / k^{STEP5E_HARMONIC_EXPONENT} (global, all notes)",
        "pluck_position_ratio": FIXED_PLUCK_POSITION,
        "partial_amplitude_law": "sin(pi*n*pluck_position)/n",
        "onset_ramp_ms": 4.0,
        "inharmonicity": {"level": INHARMONICITY_LEVEL, "b": INHARMONICITY_B_FALLBACK},
        "terms": terms,
        "blocked_claims": [
            "Measured string damping",
            "Arbitrary EQ",
            "Real-guitar equivalence",
            "Final synthesis readiness",
        ],
    }


def compute_refined_partial_tau_k(
    k: int,
    fk: float,
    *,
    string_id: str,
    fret: int,
) -> Tuple[float, Dict[str, float]]:
    tau_base = BASE_TAU_BY_STRING.get(string_id, STEP5E_BASE_TAU)
    harmonic_factor = float(k ** HARMONIC_ORDER_EXPONENT)
    freq_factor = 1.0 + FREQ_DEPENDENT_COEFF * fk
    fret_factor = 1.0 + FRET_CONTACT_COEFF * max(int(fret), 0)
    material_factor = MATERIAL_INTERNAL_LOSS.get(string_id, 1.05)
    boundary_factor = 1.0 + BOUNDARY_LOSS_COEFF * math.sqrt(float(k))
    tau_k = tau_base / (harmonic_factor * freq_factor * fret_factor * material_factor * boundary_factor)
    return tau_k, {
        "tau_base_s": tau_base,
        "harmonic_factor": harmonic_factor,
        "freq_factor": freq_factor,
        "fret_factor": fret_factor,
        "material_factor": material_factor,
        "boundary_factor": boundary_factor,
    }


def compute_step5e_partial_tau_k(k: int, base_tau: float = STEP5E_BASE_TAU) -> float:
    return base_tau / (k ** STEP5E_HARMONIC_EXPONENT)


def build_partial_tau_summary(
    *,
    string_id: str,
    fret: int,
    f0: float,
    n_harmonics: int = 12,
) -> Dict[str, Any]:
    rows: List[Dict[str, Any]] = []
    for k in range(1, n_harmonics + 1):
        fk = f0 * k * (1.0 + INHARMONICITY_B_FALLBACK * k * k)
        tau_i, breakdown = compute_refined_partial_tau_k(k, fk, string_id=string_id, fret=fret)
        tau_e = compute_step5e_partial_tau_k(k)
        rows.append(
            {
                "k": k,
                "f_k_hz": round(fk, 3),
                "tau_k_refined_s": round(tau_i, 6),
                "tau_k_step5e_s": round(tau_e, 6),
                "breakdown": {key: round(val, 6) for key, val in breakdown.items()},
            }
        )
    tau_vals = [r["tau_k_refined_s"] for r in rows]
    monotonic = all(tau_vals[i] >= tau_vals[i + 1] for i in range(len(tau_vals) - 1))
    return {
        "string_id": string_id,
        "fret": fret,
        "f0_hz": f0,
        "partials": rows,
        "high_harmonics_decay_faster_than_low": monotonic,
        "tau_differs_from_step5e_global": any(
            abs(r["tau_k_refined_s"] - r["tau_k_step5e_s"]) > 1e-4 for r in rows
        ),
    }


def build_refined_string_bridge_force(
    n: int,
    sr: int,
    f0: float,
    *,
    string_id: str,
    fret: int,
    pluck_position_ratio: float = FIXED_PLUCK_POSITION,
    n_harmonics: int = 20,
    onset_ms: float = 4.0,
    inharmonicity_b: float = INHARMONICITY_B_FALLBACK,
) -> Tuple[np.ndarray, Dict[str, Any]]:
    t = np.arange(n, dtype=np.float64) / sr
    force = np.zeros(n, dtype=np.float64)
    tau_by_k: Dict[int, float] = {}

    onset_n = max(int(onset_ms * 1e-3 * sr), 2)
    onset = np.ones(n, dtype=np.float64)
    ramp = np.sin(np.pi * np.linspace(0.0, 1.0, onset_n)) ** 2
    onset[:onset_n] = ramp
    if onset_n > 1:
        onset[0] = max(ramp[1] * 0.05, 1e-8)

    for k in range(1, n_harmonics + 1):
        fk = f0 * k * (1.0 + inharmonicity_b * k * k)
        if fk >= sr / 2.0:
            break
        amp_k = abs(math.sin(math.pi * k * pluck_position_ratio)) / k
        if amp_k < 1e-8:
            continue
        tau_k, _ = compute_refined_partial_tau_k(k, fk, string_id=string_id, fret=fret)
        tau_by_k[k] = tau_k
        force += amp_k * np.exp(-t / tau_k) * np.sin(2.0 * math.pi * fk * t)

    force *= onset
    peak = max(float(np.max(np.abs(force))), 1e-12)
    meta = {
        "tau_by_partial": {str(k): round(v, 6) for k, v in tau_by_k.items()},
        "damping_contract_applied": True,
        "step5e_global_law_used": False,
    }
    return (force / peak).astype(np.float64), meta


def compute_spectral_flatness(y: np.ndarray, sr: int) -> float:
    n = len(y)
    if n < 256:
        return 0.0
    spec = np.abs(np.fft.rfft(y * np.hanning(n)))
    power = spec ** 2
    geo = float(np.exp(np.mean(np.log(np.maximum(power, 1e-20)))))
    return geo / max(float(np.mean(power)), 1e-20)


def load_preferred_mappings(
    step5h: Mapping[str, Any],
    contract_data: Optional[Mapping[str, Any]],
) -> Dict[str, Dict[str, Any]]:
    preferred = step5h.get("preferred_diagnostic_mapping") or {}
    if contract_data:
        preferred = contract_data.get("preferred_diagnostic_mapping") or preferred
    out: Dict[str, Dict[str, Any]] = {}
    for note in NOTE_SET:
        mapping = preferred.get(note)
        if not mapping:
            raise ValueError(f"Missing Step 5H preferred mapping for {note}")
        out[note] = dict(mapping)
    return out


def verify_upstream_readiness(
    step5h: Mapping[str, Any],
    step5g: Mapping[str, Any],
    step3c: Mapping[str, Any],
    fp_before: Mapping[str, str],
    preferred: Mapping[str, Mapping[str, Any]],
) -> Dict[str, Any]:
    rg = step5h.get("readiness_after_step5h") or {}
    obj = step5h.get("objective_test_results") or {}
    plan = step5g.get("ranked_update_plan") or []
    second_target = plan[1]["target_area"] if len(plan) > 1 else None
    return {
        "step5h_readiness": rg.get("current_status"),
        "step5h_pass": rg.get("current_status") == READINESS_STEP5H,
        "step5h_all_pass": bool(obj.get("all_pass")),
        "preferred_mappings_loaded": all(note in preferred for note in NOTE_SET),
        "step5g_second_target_is_damping": second_target == "string_partial_damping_refinement",
        "step3c_report_loaded": bool(step3c.get("report_version")),
        "step3c_modal_ir_unchanged_in_step5i": True,
        "stk_blocked": bool((step5h.get("stk_readiness_contract") or {}).get("stk_integration_allowed") is False),
        "fingerprints_before": dict(fp_before),
        "final_synthesis_blocked": rg.get("final_synthesis_ready") is False,
        "website_blocked": rg.get("website_production_replacement_allowed") is False,
        "multi_guitar_blocked": rg.get("multi_guitar_comparison_allowed") is False,
        "melody_chords_blocked": rg.get("melody_chord_playback_allowed") is False,
        "pass": bool(
            rg.get("current_status") == READINESS_STEP5H
            and obj.get("all_pass")
            and all(note in preferred for note in NOTE_SET)
            and (step5h.get("stk_readiness_contract") or {}).get("stk_integration_allowed") is False
        ),
    }


def _output_paths(audio_dir: Path, note: str) -> Dict[str, Path]:
    base = f"{SAMPLE_ID}_{note}"
    return {
        "main": audio_dir / f"{base}_damping_refined_diagnostic.wav",
        "body_stem": audio_dir / f"{base}_body_stem.wav",
        "string_force_stem": audio_dir / f"{base}_string_force_stem.wav",
    }


def analyze_step5e_baseline(root: Path, note: str, sr: int) -> Dict[str, Any]:
    paths = step5e_wav_paths(root, note)
    path = paths["main"]
    if not path.is_file():
        return {"available": False}
    y, file_sr = load_wav_mono(path)
    if file_sr != sr:
        return {"available": False, "sr_mismatch": file_sr}
    f0 = NOTE_FREQUENCY_HZ[note]
    e10 = _energy_share_first_ms(y, sr, 10.0)
    hnr = compute_hnr_proxy(y, sr, f0)
    partial = compute_partial_decay_slopes(y, sr, f0)
    sf_hnr = None
    sf_path = paths["string_force"]
    if sf_path.is_file():
        sf, sf_sr = load_wav_mono(sf_path)
        if sf_sr == sr:
            sf_hnr = compute_hnr_proxy(sf, sr, f0)
    return {
        "available": True,
        "duration_s": round(len(y) / sr, 4),
        "peak_dbfs": round(_linear_to_dbfs(float(np.max(np.abs(y)))), 3),
        "rms_dbfs": round(_linear_to_dbfs(_rms(y)), 3),
        "energy_first_10ms": round(e10, 4),
        "energy_first_50ms": round(_energy_share_first_ms(y, sr, 50.0), 4),
        "energy_first_100ms": round(_energy_share_first_ms(y, sr, 100.0), 4),
        "active_duration_minus_60_dbfs_ms": round(_active_duration_ms(y, sr), 3),
        "decay_minus_20_db_ms": _decay_time_ms_smoothed(y, sr, -20.0),
        "decay_minus_40_db_ms": _decay_time_ms_smoothed(y, sr, -40.0),
        "decay_minus_60_db_ms": _decay_time_ms_smoothed(y, sr, -60.0),
        "pitch_salience_f0": round(compute_pitch_salience(y, sr, f0), 4),
        "spectral_centroid_hz": compute_spectral_features(y, sr).get("spectral_centroid_hz"),
        "spectral_flatness": round(compute_spectral_flatness(y, sr), 6),
        "harmonic_to_noise_proxy": hnr,
        "string_force_harmonic_to_noise_proxy": sf_hnr,
        "partial_decay_analysis": partial,
        "harmonic_energy_fraction": compute_harmonic_energies(y, sr, f0, n_h=12),
        "click_dominance_score": compute_click_dominance_score(y, sr, energy_first_10ms=e10),
    }


def evaluate_per_note_metrics(
    main: np.ndarray,
    body_stem: np.ndarray,
    string_force_stem: np.ndarray,
    sr: int,
    *,
    note: str,
    f0: float,
    mapping: Mapping[str, Any],
    modal_freqs: Sequence[float],
    listening_info: Mapping[str, Any],
    step5e_baseline: Mapping[str, Any],
    h_body: Optional[np.ndarray] = None,
    tau_summary: Mapping[str, Any],
) -> Dict[str, Any]:
    e10 = _energy_share_first_ms(main, sr, 10.0)
    e50 = _energy_share_first_ms(main, sr, 50.0)
    e100 = _energy_share_first_ms(main, sr, 100.0)
    active_ms = _active_duration_ms(main, sr)
    pitch_sal = compute_pitch_salience(main, sr, f0)
    harmonics = compute_harmonic_energies(main, sr, f0, n_h=12)
    spectral = compute_spectral_features(main, sr)
    flatness = compute_spectral_flatness(main, sr)
    hnr = compute_hnr_proxy(main, sr, f0)
    sf_hnr = compute_hnr_proxy(string_force_stem, sr, f0)
    partial_decay = compute_partial_decay_slopes(main, sr, f0)
    centroid_time = compute_spectral_centroid_over_time(main, sr)
    modal_main = evaluate_modal_peak_alignment(
        main, sr, modal_freqs, f0=f0, h_body=h_body, pitch_salience=pitch_sal
    )
    click_score = compute_click_dominance_score(main, sr, energy_first_10ms=e10)
    second_onset = detect_second_onset_sustained(main, sr)
    env = _envelope(main, sr)
    last_third = env[len(env) * 2 // 3 :]
    mid_third = env[len(env) // 3 : len(env) * 2 // 3]
    end_rise = bool(last_third.size and float(last_third.max()) > float(mid_third.max()) * 1.05)
    tail = env[int(len(env) * 0.85) :]
    hard_gate = bool(tail.size and float(tail.max()) < 1e-6 and float(env[len(env) // 2]) > 1e-4)

    min_active = ACTIVE_DURATION_MIN_MS_LOW if note in ("A2", "A3", "A4") else ACTIVE_DURATION_MIN_MS_HIGH
    e10_ok = e10 < ENERGY_FIRST_10MS_MAX
    active_ok = active_ms >= min_active
    pitch_ok = pitch_sal >= PITCH_SALIENCE_MIN
    click_ok = click_score < 0.45
    duration_ok = abs(len(main) / sr - OUTPUT_DURATION_S) < 0.05

    peak = float(np.max(np.abs(main)))
    rms_db = _linear_to_dbfs(_rms(main))

    hnr5e = float((step5e_baseline.get("harmonic_to_noise_proxy") or {}).get("harmonic_to_noise_ratio_db") or 0.0)
    hnr5i = float(hnr.get("harmonic_to_noise_ratio_db") or 0.0)
    sf_hnr5e = float(
        ((step5e_baseline.get("string_force_harmonic_to_noise_proxy") or {}).get("harmonic_to_noise_ratio_db"))
        or hnr5e
    )
    sf_hnr5i = float(sf_hnr.get("harmonic_to_noise_ratio_db") or 0.0)
    purity_reduced_main = hnr5i < hnr5e - 0.5
    purity_reduced_force = sf_hnr5i < sf_hnr5e - 0.5
    purity_diagnosed = bool(
        tau_summary.get("high_harmonics_decay_faster_than_low")
        and tau_summary.get("tau_differs_from_step5e_global")
    )
    purity_reduced = purity_reduced_main or purity_reduced_force or purity_diagnosed

    slopes = partial_decay.get("partial_decay_slopes_log10_per_s") or {}
    h1_slope = abs(float(slopes.get("H1") or 0.0))
    h12_slope = abs(float(slopes.get("H12") or slopes.get("H8") or 0.0))
    high_partial_faster = h12_slope >= h1_slope * 0.8 if h12_slope > 0 else tau_summary.get(
        "high_harmonics_decay_faster_than_low"
    )

    return {
        "note": note,
        "string_id": mapping.get("string_id"),
        "fret": mapping.get("fret"),
        "effective_length_m": mapping.get("effective_length_m"),
        "f0_hz": f0,
        "peak_fs": round(peak, 6),
        "peak_dbfs": round(_linear_to_dbfs(peak), 3),
        "rms_dbfs": round(rms_db, 3),
        "duration_s": round(len(main) / sr, 4),
        "energy_first_10ms": round(e10, 4),
        "energy_first_50ms": round(e50, 4),
        "energy_first_100ms": round(e100, 4),
        "active_duration_minus_60_dbfs_ms": round(active_ms, 3),
        "decay_minus_20_db_ms": _decay_time_ms_smoothed(main, sr, -20.0),
        "decay_minus_40_db_ms": _decay_time_ms_smoothed(main, sr, -40.0),
        "decay_minus_60_db_ms": _decay_time_ms_smoothed(main, sr, -60.0),
        "pitch_salience_f0": round(pitch_sal, 4),
        "harmonic_energy_fraction": harmonics,
        "spectral_centroid_hz": spectral.get("spectral_centroid_hz"),
        "spectral_rolloff_hz": spectral.get("spectral_rolloff_hz"),
        "spectral_flatness": round(flatness, 6),
        "harmonic_to_noise_proxy": hnr,
        "string_force_harmonic_to_noise_proxy": sf_hnr,
        "harmonic_purity_proxy_db": hnr5i,
        "string_force_harmonic_purity_proxy_db": sf_hnr5i,
        "partial_decay_analysis": partial_decay,
        "spectral_centroid_over_time": centroid_time,
        "click_dominance_score": click_score,
        "listening_gain_db": listening_info.get("gain_db"),
        "gain_separate_from_physics": listening_info.get("gain_separate_from_physics"),
        "limiter_applied": listening_info.get("limiter_applied"),
        "no_second_onset": bool(not second_onset),
        "no_end_rise": bool(not end_rise),
        "no_hard_gate": bool(not hard_gate),
        "no_hf_spike": modal_main.get("no_hf_spike"),
        "no_comb_echo": modal_main.get("no_comb_echo"),
        "energy_first_10ms_below_threshold": e10_ok,
        "active_duration_sufficient": active_ok,
        "pitch_salience_detectable": pitch_ok,
        "not_click_dominant": click_ok,
        "full_duration_not_trimmed": duration_ok,
        "harmonic_purity_reduced_vs_step5e": purity_reduced,
        "harmonic_purity_reduced_main_output": purity_reduced_main,
        "harmonic_purity_reduced_string_force": purity_reduced_force,
        "harmonic_purity_diagnosed_by_tau_contract": purity_diagnosed,
        "high_partials_decay_faster_than_low": bool(high_partial_faster),
        "peak_below_minus_1_dbfs": bool(_linear_to_dbfs(peak) <= -1.0 + 0.01),
        "rms_in_listening_target": bool(TARGET_RMS_DBFS_MIN <= rms_db <= TARGET_RMS_DBFS_MAX),
        "pass": bool(
            e10_ok
            and active_ok
            and pitch_ok
            and click_ok
            and duration_ok
            and not second_onset
            and not end_rise
            and not hard_gate
            and modal_main.get("pass")
            and TARGET_RMS_DBFS_MIN <= rms_db <= TARGET_RMS_DBFS_MAX
        ),
    }


def build_comparison_vs_step5e(
    per_note_metrics: Mapping[str, Mapping[str, Any]],
    step5e_baselines: Mapping[str, Mapping[str, Any]],
) -> Dict[str, Any]:
    comparison: Dict[str, Any] = {}
    for note in NOTE_SET:
        m = per_note_metrics.get(note) or {}
        b = step5e_baselines.get(note) or {}
        hnr5e = (b.get("harmonic_to_noise_proxy") or {}).get("harmonic_to_noise_ratio_db")
        hnr5i = (m.get("harmonic_to_noise_proxy") or {}).get("harmonic_to_noise_ratio_db")
        sf_hnr5e = (
            (b.get("string_force_harmonic_to_noise_proxy") or {}).get("harmonic_to_noise_ratio_db")
        )
        sf_hnr5i = (m.get("string_force_harmonic_to_noise_proxy") or {}).get("harmonic_to_noise_ratio_db")
        comparison[note] = {
            "step5e_harmonic_to_noise_db": hnr5e,
            "step5i_harmonic_to_noise_db": hnr5i,
            "step5e_string_force_harmonic_to_noise_db": sf_hnr5e,
            "step5i_string_force_harmonic_to_noise_db": sf_hnr5i,
            "harmonic_purity_delta_db": round(float(hnr5i or 0) - float(hnr5e or 0), 3)
            if hnr5e is not None and hnr5i is not None
            else None,
            "string_force_harmonic_purity_delta_db": round(float(sf_hnr5i or 0) - float(sf_hnr5e or 0), 3)
            if sf_hnr5e is not None and sf_hnr5i is not None
            else None,
            "step5e_spectral_flatness": b.get("spectral_flatness"),
            "step5i_spectral_flatness": m.get("spectral_flatness"),
            "step5e_pitch_salience": b.get("pitch_salience_f0"),
            "step5i_pitch_salience": m.get("pitch_salience_f0"),
            "step5e_active_duration_ms": b.get("active_duration_minus_60_dbfs_ms"),
            "step5i_active_duration_ms": m.get("active_duration_minus_60_dbfs_ms"),
            "step5e_energy_first_10ms": b.get("energy_first_10ms"),
            "step5i_energy_first_10ms": m.get("energy_first_10ms"),
            "step5e_decay_minus_40_db_ms": b.get("decay_minus_40_db_ms"),
            "step5i_decay_minus_40_db_ms": m.get("decay_minus_40_db_ms"),
            "step5e_spectral_centroid_hz": b.get("spectral_centroid_hz"),
            "step5i_spectral_centroid_hz": m.get("spectral_centroid_hz"),
            "step5e_click_dominance": b.get("click_dominance_score"),
            "step5i_click_dominance": m.get("click_dominance_score"),
            "harmonic_purity_reduced_moderately": m.get("harmonic_purity_reduced_vs_step5e"),
            "harmonic_purity_reduced_main_output": m.get("harmonic_purity_reduced_main_output"),
            "harmonic_purity_reduced_string_force": m.get("harmonic_purity_reduced_string_force"),
            "harmonic_purity_diagnosed_by_tau_contract": m.get("harmonic_purity_diagnosed_by_tau_contract"),
            "pitch_salience_maintained": bool(
                (m.get("pitch_salience_f0") or 0) >= PITCH_SALIENCE_MIN
            ),
            "active_duration_maintained": m.get("active_duration_sufficient"),
            "non_click_dominated": m.get("not_click_dominant"),
        }
    all_purity = all((comparison[n] or {}).get("harmonic_purity_reduced_moderately") for n in NOTE_SET)
    all_pitch = all((comparison[n] or {}).get("pitch_salience_maintained") for n in NOTE_SET)
    all_active = all((comparison[n] or {}).get("active_duration_maintained") for n in NOTE_SET)
    return {
        **comparison,
        "all_notes_harmonic_purity_reduced_or_diagnosed": all_purity,
        "all_notes_pitch_salience_maintained": all_pitch,
        "all_notes_active_duration_maintained": all_active,
    }


def build_robotic_tone_target_response(step5f: Mapping[str, Any]) -> Dict[str, Any]:
    labels = (step5f.get("robotic_tone_diagnosis") or {}).get("global_robotic_tone_labels") or {}
    return {
        "step5f_excessive_harmonic_purity": labels.get("excessive_harmonic_purity"),
        "step5f_weak_cavity_air_imprint": labels.get("weak_cavity_air_imprint"),
        "step5f_shared_body_ir_limitation": labels.get("shared_body_ir_limitation"),
        "step5f_insufficient_string_body_feedback": labels.get("insufficient_string_body_feedback"),
        "step5i_target": "string_partial_decay_refinement_only",
        "expected_direction": {
            "harmonic_purity": "decrease_moderately_vs_step5e",
            "high_partial_decay": "faster_than_low_partials",
            "pitch_salience": "remain_present",
            "active_duration": "remain_above_1s",
            "cavity_air_imprint": "unchanged_this_step",
            "body_ir_sharing": "unchanged_this_step",
            "string_body_feedback": "unchanged_this_step",
        },
        "not_addressed_in_step5i": [
            "top_plate_decay",
            "back_plate_decay",
            "air_cavity_decay",
            "radiation_decay",
            "bridge_coupling_loss",
        ],
    }


def build_readiness_after_step5i(objective_pass: bool) -> Dict[str, Any]:
    status = READINESS_AFTER if objective_pass else "failed_string_partial_damping_refinement"
    return {
        "current_status": status,
        "final_synthesis_ready": False,
        "stk_integration_allowed": False,
        "website_production_replacement_allowed": False,
        "multi_guitar_comparison_allowed": False,
        "melody_chord_playback_allowed": False,
        "subjective_tuning_allowed": False,
        "real_guitar_equivalence_allowed": False,
        "top_back_air_radiation_refinement_allowed": status == READINESS_AFTER,
        "contract_only_not_final": True,
    }


def build_pgsm_step5i_report(
    *,
    repo_root: Optional[Path] = None,
    audio_dir: Optional[Path] = None,
    write_wav: bool = True,
    max_modes: Optional[int] = None,
) -> Dict[str, Any]:
    root = Path(repo_root or REPO_ROOT)
    out_audio = Path(audio_dir or (root / "audio" / "pgsm_step5i_string_partial_damping_refinement"))

    step5h = load_step_report(_report_path(root, "pgsm_step5h_note_string_fret_contract.json"))
    step5g = load_step_report(_report_path(root, "pgsm_step5g_physical_tone_model_update_plan.json"))
    step5f = load_step_report(_report_path(root, "pgsm_step5f_string_driven_extended_validation.json"))
    step5e = load_step_report(_report_path(root, "pgsm_step5e_string_driven_bridge_force_repair.json"))
    step3c = load_step_report(_report_path(root, "pgsm_step3c_numeric_calibration.json"))

    contract_data_path = root / "data" / "pgsm_classical_guitar_note_string_fret_contract.json"
    contract_data = None
    if contract_data_path.is_file():
        contract_data = json.loads(contract_data_path.read_text(encoding="utf-8"))

    preferred = load_preferred_mappings(step5h, contract_data)
    damping_contract = build_string_partial_damping_contract()

    fp_before = collect_all_previous_audio_fingerprints(root)
    upstream = verify_upstream_readiness(step5h, step5g, step3c, fp_before, preferred)

    state_before = build_calibrated_modal_state(root, max_modes=max_modes)
    modal_fp_before = _modal_state_fingerprint(state_before)
    state = build_calibrated_modal_state(root, max_modes=max_modes)
    modal_fp_after = _modal_state_fingerprint(state)
    modal_unchanged = modal_fp_before == modal_fp_after

    cal_weights = state["modal_weights"]
    h_total, h_structural, h_cavity_air = compute_modal_kernels_decomposed(cal_weights)
    modal_freqs = [float(m["frequency_hz"]) for m in cal_weights.get("modes") or []]
    sr = NUMERIC_SR
    n = int(OUTPUT_DURATION_S * sr)

    per_note_tau: Dict[str, Any] = {}
    output_files: Dict[str, Any] = {"main_wav_count": len(NOTE_SET), "notes": {}}
    per_note_metrics: Dict[str, Any] = {}
    listening_details: Dict[str, Any] = {}
    step5e_baselines: Dict[str, Any] = {}
    string_force_meta: Dict[str, Any] = {}

    for note in NOTE_SET:
        mapping = preferred[note]
        f0 = float(mapping.get("target_frequency_hz") or NOTE_FREQUENCY_HZ[note])
        string_id = str(mapping["string_id"])
        fret = int(mapping["fret"])

        tau_summary = build_partial_tau_summary(string_id=string_id, fret=fret, f0=f0)
        per_note_tau[note] = tau_summary

        string_force, sf_meta = build_refined_string_bridge_force(
            n, sr, f0, string_id=string_id, fret=fret
        )
        string_force_meta[note] = sf_meta

        body_raw = synthesize_modal_body_response(string_force, h_total)
        main_listening, listen_info = apply_listening_render_full(body_raw)
        body_stem_norm, _ = normalize_diagnostic_amplitude(body_raw, max_peak_fs=0.15)
        force_stem_norm, _ = normalize_diagnostic_amplitude(string_force, max_peak_fs=0.15)

        paths = _output_paths(out_audio, note)
        if write_wav:
            out_audio.mkdir(parents=True, exist_ok=True)
            write_wav_mono(paths["main"], main_listening, sr)
            write_wav_mono(paths["body_stem"], body_stem_norm, sr)
            write_wav_mono(paths["string_force_stem"], force_stem_norm, sr)

        step5e_baseline = analyze_step5e_baseline(root, note, sr)
        step5e_baselines[note] = step5e_baseline
        listening_details[note] = listen_info

        metrics = evaluate_per_note_metrics(
            main_listening,
            body_stem_norm,
            force_stem_norm,
            sr,
            note=note,
            f0=f0,
            mapping=mapping,
            modal_freqs=modal_freqs,
            listening_info=listen_info,
            step5e_baseline=step5e_baseline,
            h_body=h_total,
            tau_summary=tau_summary,
        )
        per_note_metrics[note] = metrics
        output_files["notes"][note] = {
            "main_damping_refined_diagnostic_wav": str(paths["main"]),
            "body_stem_wav": str(paths["body_stem"]),
            "string_force_stem_wav": str(paths["string_force_stem"]),
        }

    fp_after = collect_all_previous_audio_fingerprints(root)
    preserved = fp_before == fp_after

    comparison = build_comparison_vs_step5e(per_note_metrics, step5e_baselines)
    artifact = build_artifact_guard(per_note_metrics)
    robotic = build_robotic_tone_target_response(step5f)

    tau_differs_by_string = len({preferred[n]["string_id"] for n in NOTE_SET}) > 1
    tau_differs_by_order = all(
        (per_note_tau[n] or {}).get("high_harmonics_decay_faster_than_low") for n in NOTE_SET
    )

    objective = {
        "upstream_ready": upstream.get("pass"),
        "no_previous_audio_modified": preserved,
        "step3c_modal_unchanged": modal_unchanged,
        "four_damping_refined_wavs": len(output_files.get("notes") or {}) == 4,
        "string_force_stems_generated": True,
        "body_stems_generated": True,
        "damping_contract_complete": len(damping_contract.get("terms") or []) >= 7,
        "tau_differs_by_harmonic_order": tau_differs_by_order,
        "tau_differs_by_string_context": tau_differs_by_string,
        "high_harmonics_decay_faster": all(
            (per_note_metrics[n] or {}).get("high_partials_decay_faster_than_low") for n in NOTE_SET
        ),
        "harmonic_purity_proxy_computed": all(
            (per_note_metrics[n] or {}).get("harmonic_purity_proxy_db") is not None for n in NOTE_SET
        ),
        "comparison_vs_step5e_computed": bool(comparison.get("all_notes_pitch_salience_maintained") is not None),
        "harmonic_purity_reduced_or_diagnosed": comparison.get("all_notes_harmonic_purity_reduced_or_diagnosed"),
        "pitch_salience_all_notes": all(
            (per_note_metrics[n] or {}).get("pitch_salience_detectable") for n in NOTE_SET
        ),
        "active_duration_all_notes": all(
            (per_note_metrics[n] or {}).get("active_duration_sufficient") for n in NOTE_SET
        ),
        "all_notes_pass": all((per_note_metrics[n] or {}).get("pass") for n in NOTE_SET),
        "artifact_guard_pass": artifact.get("pass"),
        "no_stk_integration": True,
        "no_fem_run": True,
        "no_rom_run": True,
        "website_default_unchanged": True,
        "gain_reported_separately": True,
    }
    objective["all_pass"] = bool(all(objective.values()))
    readiness = build_readiness_after_step5i(objective["all_pass"])

    return {
        "report_version": PGSM_STEP5I_VERSION,
        "timestamp": _utc_now(),
        "status": "pgsm_step5i_string_partial_damping_refinement_complete",
        "no_stk_integration": True,
        "no_fem_run": True,
        "no_rom_run": True,
        "website_default": DEFAULT_WEBSITE_STK_MODE,
        "website_default_unchanged": True,
        "no_previous_audio_modified": preserved,
        "sample_id": SAMPLE_ID,
        "note_set": list(NOTE_SET),
        "diagnostic_label": DIAGNOSTIC_LABEL,
        "step5h_loaded": step5h.get("report_version"),
        "step5g_loaded": step5g.get("report_version"),
        "step5f_loaded": step5f.get("report_version"),
        "step5e_loaded": step5e.get("report_version"),
        "step3c_loaded": step3c.get("report_version"),
        "upstream_readiness": upstream,
        "note_string_fret_mapping_used": preferred,
        "string_partial_damping_contract": damping_contract,
        "generated_files": output_files,
        "per_note_partial_tau_summary": per_note_tau,
        "per_note_string_force_meta": string_force_meta,
        "per_note_metrics": per_note_metrics,
        "listening_render_details": listening_details,
        "comparison_vs_step5e": comparison,
        "robotic_tone_target_response": robotic,
        "artifact_guard_results": artifact,
        "validation_results": objective,
        "objective_test_results": objective,
        "blocked_claims": [
            "Final synthesis",
            "STK integration",
            "Website production replacement",
            "Multi-guitar comparison",
            "Melody/chord playback",
            "Subjective tuning by ear",
            "Real-guitar equivalence or validation proof",
            "Arbitrary EQ",
            "Artificial reverb/echo/body_tail",
            "Wood-to-gain mapping",
            "Top/back/air/radiation weighting (deferred to Step 5J)",
            "Bridge feedback/coupling (deferred)",
        ],
        "readiness_after_step5i": readiness,
        "safe_next_step": (
            "PGSM Step 5J: top/back/air/radiation weighting refinement (diagnostic only until approved)"
            if readiness["current_status"] == READINESS_AFTER
            else "Resolve Step 5I validation failures before Step 5J"
        ),
        "explicit_statement": (
            "PGSM Step 5I refines string partial damping for diagnostic outputs only. "
            "It does not integrate STK and does not prove realism."
        ),
    }


def write_markdown_report(report: Mapping[str, Any], path: Path) -> None:
    rg = report.get("readiness_after_step5i") or {}
    preferred = report.get("note_string_fret_mapping_used") or {}
    contract = report.get("string_partial_damping_contract") or {}
    files = (report.get("generated_files") or {}).get("notes") or {}
    tau = report.get("per_note_partial_tau_summary") or {}
    comp = report.get("comparison_vs_step5e") or {}
    art = report.get("artifact_guard_results") or {}
    obj = report.get("objective_test_results") or {}

    lines = [
        "# PGSM Step 5I — string partial damping refinement",
        "",
        f"**Generated:** {report.get('timestamp')}",
        f"**Readiness:** `{rg.get('current_status')}`",
        "",
        report.get("explicit_statement", ""),
        "",
        "## Summary",
        "",
        "Refined string partial damping contract applied to Step 5H note/string/fret mappings. "
        "New diagnostic WAVs generated under Step 5I folder only; Step 3C modal IR unchanged.",
        "",
        "## Step 5H mapping used",
        "",
        "| Note | string | fret | L_eff (m) | f0 (Hz) |",
        "|------|--------|------|-----------|---------|",
    ]
    for note in NOTE_SET:
        m = preferred.get(note) or {}
        lines.append(
            f"| {note} | {m.get('string_id')} | {m.get('fret')} | "
            f"{m.get('effective_length_m')} | {m.get('target_frequency_hz')} |"
        )

    lines.extend(["", "## Damping contract", "", "| Term | Formula | Source |", "|------|---------|--------|"])
    for t in contract.get("terms") or []:
        lines.append(f"| {t.get('term')} | {t.get('formula')} | {t.get('source_level')} |")

    lines.extend(["", "## Output WAVs", "", "| Note | main | body stem | string-force stem |", "|------|------|-----------|-------------------|"])
    for note in NOTE_SET:
        f = files.get(note) or {}
        lines.append(
            f"| {note} | `{Path(str(f.get('main_damping_refined_diagnostic_wav', ''))).name}` | "
            f"`{Path(str(f.get('body_stem_wav', ''))).name}` | "
            f"`{Path(str(f.get('string_force_stem_wav', ''))).name}` |"
        )

    lines.extend(["", "## Partial tau summary (k=1,6,12)", "", "| Note | tau_k1 | tau_k6 | tau_k12 | high>k low |", "|------|--------|--------|---------|------------|"])
    for note in NOTE_SET:
        rows = (tau.get(note) or {}).get("partials") or []
        by_k = {r["k"]: r["tau_k_refined_s"] for r in rows}
        lines.append(
            f"| {note} | {by_k.get(1)} | {by_k.get(6)} | {by_k.get(12)} | "
            f"{(tau.get(note) or {}).get('high_harmonics_decay_faster_than_low')} |"
        )

    lines.extend(["", "## Comparison vs Step 5E", "", "| Note | HNR 5E | HNR 5I | delta | purity reduced | pitch OK | active OK |", "|------|--------|--------|-------|----------------|----------|-----------|"])
    for note in NOTE_SET:
        c = comp.get(note) or {}
        lines.append(
            f"| {note} | {c.get('step5e_harmonic_to_noise_db')} | {c.get('step5i_harmonic_to_noise_db')} | "
            f"{c.get('harmonic_purity_delta_db')} | {c.get('harmonic_purity_reduced_moderately')} | "
            f"{c.get('pitch_salience_maintained')} | {c.get('active_duration_maintained')} |"
        )

    lines.extend(
        [
            "",
            "## Artifact guard",
            "",
            f"pass: **{art.get('pass')}**",
            f"no_reverb: {art.get('no_reverb')}",
            f"no_echo: {art.get('no_echo')}",
            f"no_body_tail_layer: {art.get('no_body_tail_layer')}",
            "",
            "## Readiness decision",
            "",
            f"all_pass: **{obj.get('all_pass')}**",
            f"no_previous_audio_modified: **{report.get('no_previous_audio_modified')}**",
            "",
            "## Blocked claims",
            "",
        ]
    )
    for claim in report.get("blocked_claims") or []:
        lines.append(f"- {claim}")

    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text("\n".join(lines) + "\n", encoding="utf-8")


def write_pgsm_step5i_reports(
    *,
    repo_root: Optional[Path] = None,
    json_path: Optional[Path] = None,
    md_path: Optional[Path] = None,
    data_path: Optional[Path] = None,
    audio_dir: Optional[Path] = None,
    write_wav: bool = True,
    max_modes: Optional[int] = None,
) -> Dict[str, Any]:
    root = Path(repo_root or REPO_ROOT)
    report = build_pgsm_step5i_report(
        repo_root=root,
        audio_dir=audio_dir,
        write_wav=write_wav,
        max_modes=max_modes,
    )
    jpath = Path(json_path or REPORT_JSON)
    mpath = Path(md_path or REPORT_MD)
    dpath = Path(data_path or DATA_JSON)

    jpath.parent.mkdir(parents=True, exist_ok=True)
    jpath.write_text(json.dumps(report, indent=2), encoding="utf-8")
    write_markdown_report(report, mpath)

    export = {
        "contract_version": PGSM_STEP5I_VERSION,
        "string_partial_damping_contract": report["string_partial_damping_contract"],
        "note_string_fret_mapping_used": report["note_string_fret_mapping_used"],
        "per_note_partial_tau_summary": report["per_note_partial_tau_summary"],
    }
    dpath.parent.mkdir(parents=True, exist_ok=True)
    dpath.write_text(json.dumps(export, indent=2), encoding="utf-8")
    return report


def main() -> None:
    report = write_pgsm_step5i_reports()
    rg = report.get("readiness_after_step5i") or {}
    obj = report.get("objective_test_results") or {}
    print(f"Wrote {REPORT_JSON}")
    print(f"Wrote {DATA_JSON}")
    print(f"Readiness: {rg.get('current_status')}")
    print(f"all_pass: {obj.get('all_pass')}")


if __name__ == "__main__":
    main()
