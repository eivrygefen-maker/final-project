#!/usr/bin/env python3
"""
PGSM Step 5I.2 — string decay floor and high-note peak balance repair.
V3 partial damping + extended diagnostic window; diagnostic only.
"""
...
from pgsm_step5i_1_string_damping_duration_harshness_repair import (
    READINESS_AFTER as READINESS_STEP5I_1,
...
PGSM_STEP5I_2_VERSION = "pgsm_step5i_2_string_decay_floor_peak_balance_repair_v1"
REPO_ROOT = Path(__file__).resolve().parents[1]
REPORT_JSON = (
    REPO_ROOT / "audio" / "debug_reports" / "pgsm_step5i_2_string_decay_floor_peak_balance_repair.json"
)
REPORT_MD = (
    REPO_ROOT / "audio" / "debug_reports" / "pgsm_step5i_2_string_decay_floor_peak_balance_repair.md"
)
DATA_JSON = REPO_ROOT / "data" / "pgsm_string_partial_damping_contract_v3.json"
AUDIO_DIR = REPO_ROOT / "audio" / "pgsm_step5i_2_string_decay_floor_peak_balance_repair"
...
HARMONIC_ORDER_EXPONENT = 1.22
FREQ_DEPENDENT_COEFF = 3.2e-4
HIGH_FREQ_EXTRA_COEFF = 8.5e-4
FRET_CONTACT_COEFF = 0.042
FRET_HIGH_EXTRA_COEFF = 0.009
BOUNDARY_LOSS_COEFF = 0.048

LOW_PARTIAL_LEAK_ONSET_S = 0.75
LOW_PARTIAL_LEAK_RATE_K1 = 0.52
LEAK_K_POWER = 0.82
LEAK_SMOOTH_S = 0.28

HIGH_FRET_THRESHOLD = 5
HIGH_NOTE_LOW_PARTIAL_TAU_DIVISOR = 0.045
...
def build_duration_policy(duration_s: float = DEFAULT_DURATION_S) -> Dict[str, Any]:
    return {
        ...
        "reason": "5.5 s exposes low-partial decay floor; 4.5 s was insufficient for -20 dB on A2/A3",
        "no_trim": True,
        "no_hard_gate": True,
...
def build_string_partial_damping_contract_v3() -> Dict[str, Any]:
    terms: List[Dict[str, Any]] = [
        {
            "term": "base_string_decay_by_string_id",
            "formula": "tau_base[string_id]; bass > treble, slightly reduced vs v2",
            "values": dict(BASE_TAU_BY_STRING),
            "source_level": "L2_literature_fallback_v3",
            "objective_metric": "measurable decay over 5.5 s without collapsing sustain",
        },
        {
            "term": "harmonic_order_loss",
            "formula": f"harmonic_factor = k^{HARMONIC_ORDER_EXPONENT}",
            "source_level": "L2_diagnostic_proxy_v3",
            "objective_metric": "high k partials decay faster than low k",
        },
        {
            ...
            "source_level": "L2_diagnostic_proxy_v3",
            "objective_metric": "high-frequency partials lose energy via shorter tau",
        },
        {
            "term": "fret_contact_loss_proxy",
            "formula": (
                f"fret_factor = 1 + {FRET_CONTACT_COEFF}*fret + "
                f"{FRET_HIGH_EXTRA_COEFF}*max(0,fret-{HIGH_FRET_THRESHOLD})"
            ),
            "source_level": "L2_diagnostic_proxy_v3",
            "objective_metric": "high-fret notes damp faster",
        },
        {
            "term": "material_internal_loss_proxy",
            "formula": "material_factor = MATERIAL_INTERNAL_LOSS[string_id]",
            "source_level": "L2_nylon_classical_fallback_v3",
        ...
        },
        {
            ...
            "source_level": "L2_diagnostic_proxy_v3",
            "objective_metric": "boundary loss increases with partial order",
        },
        {
            "term": "low_partial_long_decay_control",
            "formula": (
                "tau_k capped indirectly via late_decay_leakage; bass tau_base reduced modestly vs v2"
            ),
            "source_level": "L2_diagnostic_proxy_v3",
            "limitations": "Not a hard gate; low partials retain initial sustain then leak",
            "objective_metric": "A2/A3 show measurable decay over 5.5 s",
        },
        {
            ...
            "source_level": "L2_diagnostic_proxy_v3",
            "limitations": "Smooth causal leakage representing bridge/body/radiation energy loss proxy",
            "blocked_claims": ["hard_gate", "fade_out_trick", "body_radiation_implemented"],
            "objective_metric": "final 0.5 s energy << initial 0.5 s",
        },
        {
            "term": "high_note_peak_balance_proxy",
            "formula": (
                f"extra tau divisor on k<=4 for string_1 fret>={HIGH_FRET_THRESHOLD}: "
                f"1 + {HIGH_NOTE_LOW_PARTIAL_TAU_DIVISOR}*fret"
            ),
            "source_level": "L2_diagnostic_proxy_v3",
            "limitations": "Damping only; optional diagnostic RMS target -23 dBFS for A4/E5",
            "blocked_claims": ["arbitrary_eq", "spectral_gain_cut"],
            "objective_metric": "A4/E5 piercing proxy reduced vs Step 5I.1",
        },
        {
            "term": "combined_partial_tau",
            "formula": (
                "amplitude_k(t) = A_k * exp(-t/tau_k) * exp(-leak_k*ramp(t)) * sin(2*pi*f_k*t)"
            ),
            "source_level": "L2_diagnostic_combined_proxy_v3",
            "objective_metric": "monotonic tau_k; time-varying leakage smooth and causal",
        },
    ]
    return {
        "contract_id": "pgsm_string_partial_damping_v3",
        "supersedes": "pgsm_string_partial_damping_v2",
        "implements_multi_decay_term": "string_partial_decay_only",
        "future_body_radiation_note": (
            "True natural decay floor ultimately depends on top/back/air/radiation/coupling "
            "in Step 5J/5K; not faked here."
        ),
...
def compute_v3_partial_tau_k(
    ...
    *,
    ...
    fret_high = max(0, int(fret) - HIGH_FRET_THRESHOLD)
    fret_factor = 1.0 + FRET_CONTACT_COEFF * max(int(fret), 0) + FRET_HIGH_EXTRA_COEFF * fret_high
    material_factor = MATERIAL_INTERNAL_LOSS.get(string_id, 1.10)
    boundary_factor = 1.0 + BOUNDARY_LOSS_COEFF * math.sqrt(float(k))
    peak_balance = 1.0
    if string_id == "string_1" and int(fret) >= HIGH_FRET_THRESHOLD and k <= 4:
        peak_balance = 1.0 + HIGH_NOTE_LOW_PARTIAL_TAU_DIVISOR * int(fret)
    tau_k = tau_base / (
        harmonic_factor * freq_factor * fret_factor * material_factor * boundary_factor * peak_balance
    )
...
def build_partial_tau_summary_v3(
    *,
    string_id: str,
    fret: int,
    f0: float,
    ...
    for k in range(1, n_harmonics + 1):
        ...
        tau_v3, breakdown = compute_v3_partial_tau_k(k, fk, string_id=string_id, fret=fret)
        rows.append({"k": k, "f_k_hz": round(fk, 3), "tau_k_v3_s": round(tau_v3, 6), "breakdown": breakdown})
    tau_vals = [r["tau_k_v3_s"] for r in rows]
...
def build_v3_string_bridge_force(
    n: int,
    sr: int,
    f0: float,
    *,
    ...
    for k in range(1, n_harmonics + 1):
        ...
        if amp_k < 1e-8:
            continue
        tau_k, breakdown = compute_v3_partial_tau_k(k, fk, string_id=string_id, fret=fret)
        leak_k = breakdown["leak_k_per_s"]
        decay = np.exp(-t / tau_k) * np.exp(-leak_k * leak_ramp)
        tau_by_k[k] = tau_k
        force += amp_k * decay * np.sin(2.0 * math.pi * fk * t)
    ...
    return (force / peak).astype(np.float64), {
        ...
        "damping_contract_v3_applied": True,
        "late_decay_leakage_applied": True,
    }


def compute_low_partial_late_energy_ratio(y: np.ndarray, sr: int, f0: float) -> float:
    n = len(y)
    if n < sr:
        return 0.0
    early = y[: n // 3]
    ...
    return round(late_l / max(early_l, 1e-12), 6)


def compute_upper_mid_dominance_proxy(y: np.ndarray, sr: int) -> float:
    return round(_band_energy_ratio(y, sr, UPPER_MID_LO_HZ, UPPER_MID_HI_HZ), 6)


def compute_high_note_piercing_proxy(
    y: np.ndarray,
    sr: int,
    f0: float,
    *,
    ...
    upper_mid = compute_upper_mid_dominance_proxy(y, sr)
    high_gt2k = _band_energy_ratio(y, sr, HIGH_FREQ_THRESHOLD_HZ, sr / 2 - 100)
    centroid = compute_spectral_centroid_over_time(y, sr)
    score = (
        0.25 * min(max(crest, 0.0) / 12.0, 1.0)
        + 0.30 * min(upper_mid / 0.45, 1.0)
        + 0.20 * min(high_gt2k / 0.12, 1.0)
        + 0.15 * min(max(hnr_db, 0.0) / 50.0, 1.0)
        + 0.10 * min(float(centroid.get("centroid_drift_hz") or 0) / 200.0, 1.0)
    )
    return {
        "high_note_piercing_proxy": round(float(score), 4),
        "crest_factor_db": round(crest, 3),
        "upper_mid_dominance_proxy": upper_mid,
        "high_band_gt2k_fraction": round(high_gt2k, 6),
        "spectral_centroid_drift_hz": centroid.get("centroid_drift_hz"),
    }


def compute_decay_metrics(y: np.ndarray, sr: int, duration_s: float) -> Dict[str, Any]:
    env = _envelope(y, sr)
    t20 = _decay_time_ms_smoothed(y, sr, -20.0)
    t40 = _decay_time_ms_smoothed(y, sr, -40.0)
    t60 = _decay_time_ms_smoothed(y, sr, -60.0)
    dur_ms = duration_s * 1000.0

    half = int(0.5 * sr)
    e_init = float(np.sum(y[:half].astype(np.float64) ** 2))
    e_final = float(np.sum(y[-half:].astype(np.float64) ** 2))
    final_ratio = e_final / max(e_init, 1e-12)

    last_env = env[-int(0.5 * sr) :] if env.size else env
    residual = float(np.max(last_env)) if last_env.size else 0.0
    peak_env = float(np.max(env)) if env.size else 1e-12

    minus_20_reached = t20 is not None and t20 <= dur_ms
    minus_40_reached = t40 is not None and t40 <= dur_ms
    minus_60_reached = t60 is not None and t60 <= dur_ms

    last_third = env[len(env) * 2 // 3 :] if env.size else env
    mid_third = env[len(env) // 3 : len(env) * 2 // 3] if env.size else env
    end_rise = bool(last_third.size and mid_third.size and float(last_third.max()) > float(mid_third.max()) * 1.05)

    return {
        "t_minus_20_db_ms": t20,
        "t_minus_40_db_ms": t40,
        "t_minus_60_db_ms": t60,
        "minus_20db_reached_within_window": minus_20_reached,
        "minus_40db_reached_within_window": minus_40_reached,
        "minus_60db_reached_within_window": minus_60_reached,
        "deep_decay_pass": minus_40_reached,
        "minus_20db_honestly_flagged_not_reached": not minus_20_reached,
        "minus_40db_honestly_flagged_not_reached": not minus_40_reached,
        "final_0p5s_to_initial_0p5s_energy_ratio": round(final_ratio, 6),
        "residual_energy_final_0p5s": round(residual / max(peak_env, 1e-12), 6),
        "active_duration_minus_60_dbfs_ms": round(_active_duration_ms(y, sr), 3),
        "envelope_monotonic_no_end_rise": not end_rise,
    }
...
def verify_upstream_readiness(
    step5i_1: Mapping[str, Any],
    step5h: Mapping[str, Any],
    preferred: Mapping[str, Mapping[str, Any]],
) -> Dict[str, Any]:
    rg = step5i_1.get("readiness_after_step5i_1") or {}
    return {
        "step5i_1_readiness": rg.get("current_status"),
        "step5i_1_pass": rg.get("current_status") == READINESS_STEP5I_1,
        "step5h_mappings_present": all(note in preferred for note in NOTE_SET),
        "stk_blocked": True,
        "pass": bool(rg.get("current_status") == READINESS_STEP5I_1 and all(note in preferred for note in NOTE_SET)),
    }
...
def _output_paths(audio_dir: Path, note: str) -> Dict[str, Path]:
    base = f"{SAMPLE_ID}_{note}"
    return {
        "main": audio_dir / f"{base}_damping_v3_diagnostic.wav",
...
def evaluate_per_note(
    ...
    *,
    ...
    b51 = baselines.get("step5i_1") or {}
    piercing_ref = float((b51.get("high_note_piercing_proxy") or {}).get("high_note_piercing_proxy") or 0.0)
    piercing_new = float(piercing.get("high_note_piercing_proxy") or 0.0)
    peak_improved = piercing_new < piercing_ref - 0.02 or peak_dbfs < float(b51.get("peak_dbfs") or 99) - 0.5
    ...
    return {
        ...
        "high_note_piercing_proxy": piercing,
        "peak_balance_improved_vs_step5i_1": peak_improved,
        ...
        "not_click_dominant": compute_click_dominance_score(main, sr, energy_first_10ms=e10) < 0.45,
...
def build_high_note_peak_analysis(
    ...
    baselines_51: Mapping[str, Mapping[str, Any]],
) -> Dict[str, Any]:
    low_notes = ("A2", "A3")
    treble = ("A4", "E5")
    ...
    for note in NOTE_SET:
        ...
        analysis[f"{note}_detail"] = {
            ...
            "improved_or_flagged": m.get("peak_balance_improved_vs_step5i_1")
            or m.get("peak_balance_not_improved_flagged"),
        }
    treble_ok = all(
        (per_note_metrics[n] or {}).get("peak_balance_improved_vs_step5i_1")
...
def build_readiness_after_step5i_2(objective_pass: bool) -> Dict[str, Any]:
    status = READINESS_AFTER if objective_pass else "failed_string_decay_floor_peak_balance_repair"
    return {
...
def build_pgsm_step5i_2_report(
    *,
    ...
    damping_v3 = build_string_partial_damping_contract_v3()
    duration_policy = build_duration_policy(duration_s)

    fp_before = collect_all_previous_audio_fingerprints(root)
    upstream = verify_upstream_readiness(step5i_1, step5h, preferred)
    ...
    for note in NOTE_SET:
        ...
        tau_summary = build_partial_tau_summary_v3(string_id=string_id, fret=fret, f0=f0)
        per_note_tau[note] = tau_summary

        string_force, _ = build_v3_string_bridge_force(n, sr, f0, string_id=string_id, fret=fret)
        ...
        baselines_5i[note] = _load_baseline(step5i_wav_paths(root, note), note, sr)
        ...
        output_files["notes"][note] = {
            "main_damping_v3_diagnostic_wav": str(paths["main"]),
    ...
    comp5i = {n: _comparison_entry(per_note_metrics[n], baselines_5i[n], ref="step5i") for n in NOTE_SET}
    comp5e = {n: _comparison_entry(per_note_metrics[n], baselines_5e[n], ref="step5e") for n in NOTE_SET}

    peak_analysis = build_high_note_peak_analysis(per_note_metrics, baselines_51)
    honest_flags = build_honest_failure_flags(per_note_metrics)
    ...
    objective = {
        ...
        "four_damping_v3_wavs": len(output_files.get("notes") or {}) == 4,
        "duration_in_range": ALLOWED_DURATION_MIN_S <= duration_s <= ALLOWED_DURATION_MAX_S,
        "v3_contract_complete": len(damping_v3.get("terms") or []) >= 10,
    ...
    readiness = build_readiness_after_step5i_2(objective["all_pass"])

    return {
        "report_version": PGSM_STEP5I_2_VERSION,
        "timestamp": _utc_now(),
        "status": "pgsm_step5i_2_string_decay_floor_peak_balance_repair_complete",
        "why_step5i_2_needed": [
            "Low partials on A2/A3 did not decay enough by 4.5 s",
            "A4/E5 still piercing from upper-mid/low-partial peak dominance",
            "None of the notes reached -40 dB within 4.5 s window",
        ],
        "no_stk_integration": True,
        "no_fem_run": True,
        "no_rom_run": True,
        ...
        "string_partial_damping_contract_v3": damping_v3,
        ...
        "readiness_after_step5i_2": readiness,
        "safe_next_step": (
            "PGSM Step 5J: top/back/air/radiation weighting refinement"
            if readiness["current_status"] == READINESS_AFTER
            else "Resolve Step 5I.2 validation failures"
        ),
        "explicit_statement": (
            "PGSM Step 5I.2 repairs diagnostic string decay floor and high-note peak balance only. "
...
def write_markdown_report(report: Mapping[str, Any], path: Path) -> None:
    rg = report.get("readiness_after_step5i_2") or {}
    contract = report.get("string_partial_damping_contract_v3") or {}
    ...
    comp = report.get("comparison_vs_step5i_1") or {}
    ...
    lines = [
        ...
        "",
        "## Why Step 5I.2 was needed",
        "",
    ]
    for item in report.get("why_step5i_2_needed") or []:
        lines.append(f"- {item}")

    lines.extend(["", "## Duration policy", "", f"Applied: **{duration.get('applied_duration_s')} s**", ""])
    lines.extend(["", "## Damping v3 contract", "", "| Term | Formula |", "|------|---------|"])
    for t in contract.get("terms") or []:
        lines.append(f"| {t.get('term')} | {t.get('formula', '')[:80]} |")

    lines.extend(["", "## Decay metrics", "", "| Note | t-20dB ms | t-40dB ms | final/initial 0.5s | -20 reached | -40 reached |", "|------|-----------|-----------|-------------------|-------------|-------------|"])
    for note in NOTE_SET:
        d = decay.get(note) or {}
        lines.append(
            f"| {note} | {d.get('t_minus_20_db_ms')} | {d.get('t_minus_40_db_ms')} | "
            f"{d.get('final_0p5s_to_initial_0p5s_energy_ratio')} | "
            f"{d.get('minus_20db_reached_within_window')} | {d.get('minus_40db_reached_within_window')} |"
        )
    ...
    for note in NOTE_SET:
        ...
        c = comp.get(note) or {}
        lines.append(
            f"- **{note}**: peak={c.get('step5i_2_peak_dbfs')} dBFS, piercing_delta={c.get('piercing_delta')}, "
...
def write_pgsm_step5i_2_reports(
    *,
    repo_root: Optional[Path] = None,
    ...
    root = Path(repo_root or REPO_ROOT)
    report = build_pgsm_step5i_2_report(
        repo_root=root,
        audio_dir=audio_dir,
        write_wav=write_wav,
        max_modes=max_modes,
    ...
    export = {
        "contract_version": PGSM_STEP5I_2_VERSION,
        "string_partial_damping_contract_v3": report["string_partial_damping_contract_v3"],
