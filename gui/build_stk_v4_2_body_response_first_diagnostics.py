#!/usr/bin/env python3
"""Stage 5.2A STK body-response-first V4.2 diagnostics (A3 default)."""
...
from stage52a_stk_v4_2_body_response_first_report import build_stage52a_body_response_first_report  # noqa: E402
from synthesis_presets import DEFAULT_SYNTHESIS_PRESET  # noqa: E402

DEFAULT_OUT = REPO / "audio" / "stk_v4_2_body_response_first_diagnostics"
DEFAULT_NOTES = "A3"
DEFAULT_MODES = (
...
def _segment_row(
    ...
    *,
    ...
    row["fundamental_stability"] = meta.get("fundamental_stability_ratio")
    if z_body is not None:
        row["body_identity_vector"] = z_body
    return row


def build_v4_2_body_response_first_diagnostics(
    *,
    repo_root: Path,
    out_dir: Path,
    ...
    build_stage52a_body_response_first_report(
        ...
        out_json=report_dir / "stage52a_stk_v4_2_body_response_first_report.json",
        out_md=report_dir / "stage52a_stk_v4_2_body_response_first_report.md",
    )
    return manifest


def main(argv: Optional[Sequence[str]] = None) -> int:
    parser = argparse.ArgumentParser(description="Stage 5.2A body-response-first V4.2 diagnostics")
